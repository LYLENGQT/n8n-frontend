              ...
              <div className="p-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
                {rec.images.map((src, i) => (
                  <div key={i} className="aspect-square rounded-lg bg-muted overflow-hidden transition-transform duration-200 hover:scale-[1.02] cursor-zoom-in">
                    ...
                    <img src={src} alt="history" className="w-full h-full object-cover" loading="lazy" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" onClick={() => setLightbox({ open: true, src, index: i, flat: rec.images })} />
                  </div>
                ))}
              </div>
            </div>
